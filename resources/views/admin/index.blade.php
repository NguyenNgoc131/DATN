                                    ...
                                    <tbody>
                                        @foreach ($datas as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="editable" id="name{{ $item['id'] }}">{{ $item['name'] }}</span>
                                                    <input type="text" class="form-control" id="nameInput{{ $item['id'] }}"
                                                        value="{{ $item['name'] }}" style="display: none;">
                                                </td>
                                                <td><input type="email" class="form-control" id="email{{ $item['id'] }}"
                                                        name="email"
                                                        value="{{ substr($item['email'], 0, 5) . str_repeat('*', strlen($item['email']) - 15) . substr($item['email'], -10) }}"
                                                        readonly>
                                                </td>
                                                <td>
                                                    <span class="editable" id="role{{ $item['id'] }}">{{ $item['role'] }}</span>
                                                    <input type="text" class="form-control" id="roleInput{{ $item['id'] }}"
                                                        value="{{ $item['role'] }}" style="display: none;">
                                                </td>
                                                <td>{{ $item['created_at'] }}</td>
                                                <td>{{ $item['updated_at'] }}</td>
                                                <td>
                                                    <button class="btn btn-warning" onclick="toggleEdit({{ $item['id'] }})"
                                                        id="editBtn{{ $item['id'] }}" style="margin-bottom: 2px;">Sửa</button>
                                                    ...
                                                    <form id="updateInfo" action="{{ route('admin.update', ['id' => $item['id']]) }}"
                                                        method="post">
                                                        @csrf
                                                        <button class="btn btn-success" onclick="saveChanges({{ $item['id'] }})"
                                                            style="display: none; margin-bottom: 2px;" id="saveBtn{{ $item['id'] }}">Lưu</button>
                                                    </form>
                                                    <button class="btn btn-info" onclick="cancelEdit({{ $item['id'] }})"
